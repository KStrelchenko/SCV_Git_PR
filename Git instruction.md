...
(❗) Процедура авторизации проводится 1 раз.

После прохождения авторизации создаем удаленный репозиторий в GitHub.com:
![Logo](1.PNG)

Для этого вверху-справа нажимаем на + выбираем *New repository*. После чего вводим название нашего репозитория. Остальные настройки оставляем по умолчанию без изменений и жмем зелёную кнопку *Create repository*
...
git branch -M main
git remote add origin https_адрес_для_отправки_локального_репозитория
git push -u origin main
```
**9.4. Извлечение содержимого удаленного репозитория в локальный**

Изменения в репозитории можно проводить не только в локальном файле, но и в удалённом на GitHub. Если необходимо "забрать" изменения из уделённого репозитория в локальный, вводим команду `git pull`

**9.5. Отправка локального репозитория в удалённый**

Чтобы отправить локальную ветку репозитория в удалённый необходимо использовать команду `git push`

**9.6. Работа с чужим удалённым репозиторием**

Чтобы начать работать с чужим проектом/удалённым репозиторием. Сперва его необходимо скопировать к себе в аккаунт на GitHub:
![Logo](2.PNG)

Для этого нажимаем справа-вверху нажимаем на кнопку *Fork*

Далее появится новое окно в котором необходимо нажать зелёную кнопку *Create fork*

После того как мы скопировали удалённый ропизторий к себе на аккаунт, необходимо создать его локальную копию. Для этого используем команду `git clone https_адрес_удалённого_репозитория` (❗)

(❗) https-адрес удаленного репозитория берется с вашего аккаунта из зелёной кнопки *Code* на сайте GitHub:
![Logo](3.PNG).

(❗) Также важно помнить, что все изменения по чужому проекту **всегда** производятся в новой, отдельной ветке!

Когда закончили работу над проектом. Необходимо закомитить изменения и отправить их на удаленный репозиторий в GitHub с помощью команды `git push`.

Далее на GitHub появляется зелёная кнопка *Compare & Pull request* С помощью которой отправляем изменения владельцу проекта для согласования правок.

![Logo](4.PNG)

Если кнопки не появилось, то отправляем изменения владельцу проекта следующим способом:

переходим на вкладку *pull request* и жмем на зелёную кнопку *New Pull request*
![Logo](5.PNG).

Далее выбираем кому отправляем запрос на изменения и нашу ветку, в которой вносили правки по проекту:
![Logo](6.PNG)
